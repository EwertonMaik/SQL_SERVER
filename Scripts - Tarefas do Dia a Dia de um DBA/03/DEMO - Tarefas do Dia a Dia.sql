-- ### ANOTACOES
Sempre ao criar um banco, definir a localização separada para seu disco de dados e log
Configuração de tamanho Inicial / Maximo e Auto Crescimento, FileGroup
Configurar - Page Verify Option - Verifica encontrar problema de corrupção de página - NONE / TORN_PAGE_DETECTION / CHECKSUM (Melhor)
DETACH e ATTACH - Desanexar Base e Anexar Base de Dados - Este processo perde o histórico de backup e log das
bases, por isso o recomendado é, deixar base off-line, mover arquivos, alterar a base para o novo caminho
dos arquivos .mdf e .ldf e depois colocar a base on-line.
